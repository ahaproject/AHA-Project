...
public class ParentSingleView extends AppCompatActivity implements ReceiveResult {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (i != null) {
            ...
            p = (Parent) i.getSerializableExtra("parent");

            if( p != null){
                consult_doctor = p.getConsult_doctor();
                id = p.getUser_id();
            }
            if (id > -1) {
    ...
    public void onReceiveResult(String resultJson) {
        try {
            ...
            p = (Parent) Constants.readAndSaveUser(output);

            consult_doctor = p.getConsult_doctor();
            setTitle(p.getUser_name());

            TextView name = (TextView) findViewById(R.id.p_name);
            name.setText(getString(R.string.name) + " : " + p.getUser_name());
            ...
            if (p.getMetas().size() > 0) {
                HashMap<String, String> parentData = p.getMetas();
...
//                        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        p = (Parent) getIntent().getSerializableExtra("parent");
        consult_doctor = (p == null)? -1 : p.getConsult_doctor();
